...
bool MarlinUI::detected() { return true; }
  ...
  void MarlinUI::draw_marlin_bootscreen() {
    ...
    constexpr u8g_uint_t text_height = MENU_FONT_HEIGHT,
                         text_width = MENU_FONT_WIDTH,
                         text_width_1 = u8g_uint_t((sizeof(KS_MARLIN_TITLE) - 2) * text_width),
                         text_width_2 = u8g_uint_t((sizeof(SHORT_BUILD_VERSION) - 1) * text_width),
                         text_width_3 = u8g_uint_t((sizeof(MARLIN_WEBSITE_URL) - 1) * text_width),
                         text_total_height = text_height * 3,
                         width = LCD_PIXEL_WIDTH,
                         height = LCD_PIXEL_HEIGHT;

    u8g_int_t txt_base, txt_offx_1, txt_offx_2, txt_offx_3;

    // Text under bitmap
    txt_offx_1 = (width - text_width_1) / 2;  // Text 1 centered
    txt_offx_2 = (width - text_width_2) / 2;  // Text 2 centered
    txt_offx_3 = (width - text_width_3) / 2;  // Text 3 centered
    txt_base = height/2 - text_total_height/2 + text_height/2;   // Even spacing looks best

    u8g.firstPage();
    do {
      set_font(FONT_MENU);
      lcd_put_u8str(txt_offx_1, txt_base, F(KS_MARLIN_TITLE));
      lcd_put_u8str(txt_offx_2, txt_base + text_height, F(SHORT_BUILD_VERSION));
      lcd_put_u8str(txt_offx_3, txt_base + 2*text_height, F(MARLIN_WEBSITE_URL));
    } while (u8g.nextPage());
  }
